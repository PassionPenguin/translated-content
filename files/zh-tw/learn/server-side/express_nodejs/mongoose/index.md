...
本文簡短介紹數據庫，以及如何搭配 Node / Express 應用，使用數據庫。接下來會演示我們如何使用 [Mongoose](https://mongoosejs.com/)，為本地圖書館提供數據庫存取。本文說明物件要求與模型如何宣告，主要的欄位型態，以及基本驗證。本文也簡短演示一些存取模型數據的主要方法。
...
NPM 套件管理器站點上，有許多 ODM / ORM 解決方案（查看 [odm](https://www.npmjs.com/search?q%3Dkeywords:odm) 和 [orm](https://www.npmjs.com/search?q%3Dkeywords:orm) 標籤的子集合！）。

在撰寫本文時，受歡迎的幾種解決方案是：

- [Mongoose](https://www.npmjs.com/package/mongoose): Mongoose 是一個 [MongoDB](https://www.mongodb.com/)對象建模工具，用於在異步環境中工作。
...
對於本地圖書館示例（以及本主題的其餘部分），我們將使用 [Mongoose ODM](https://www.npmjs.com/package/mongoose) 來訪問我們的圖書館數據。 Mongoose 是 [MongoDB](https://www.mongodb.com/company/what-is-mongodb) 的前端，MongoDB 是一個使用面向文檔數據模型的開源 [NoSQL](https://en.wikipedia.org/wiki/NoSQL) 數據庫。在 MongoDB 數據庫中，「文檔」 的 「集合」 ，[類似於](https://www.mongodb.com/docs/manual/core/databases-and-collections/#collections)關係數據庫中 「行」 的 「表」。

這種 ODM 和數據庫的結合在 Node 社區中非常流行，部分原因是文檔存儲和查詢系統，看起來非常像 JSON，因此對 JavaScript 開發人員來說很熟悉。

> **備註：** 使用 Mongoose 時，你不需要事先了解 MongoDB，但是如果你已經熟悉 MongoDB，[Mongoose documentation](https://mongoosejs.com/docs/guide.html)文檔的一部分會更易於使用和理解。
...
> **備註：** 本入門受到 npm 上的[Mongoose 快速入門](https://www.npmjs.com/package/mongoose)和[Mongoose 官方文檔](https://mongoosejs.com/docs/guide.html)的「深度影響」。
...
安裝 Mongoose 會添加所有依賴項，包括 MongoDB 數據庫驅動程序，但它不會安裝 MongoDB 。如果你想安裝一個 MongoDB 服務器，那麼你可以[從這裡下載](https://www.mongodb.com/try)各種操作系統的安裝程序，並在本地安裝。你還可以使用基於雲端的 MongoDB 實例。
...
大多數綱要型態[SchemaTypes](https://mongoosejs.com/docs/schematypes.html)（「type：」之後或字段名稱之後的描述符）都是自解釋的。例外情況是：

- `ObjectId`:表示數據庫中模型的特定實例。例如，一本書可能會使用它來表示其作者對象。這實際上將包含指定對象的唯一 ID ( `_id`) 。我們可以使用`populate()`方法，在需要時提取相關訊息。
- [Mixed](https://mongoosejs.com/docs/schematypes.html#mixed) :任意綱要型態。
...
有關選項的更多訊息，請參閱[SchemaTypes](https://mongoosejs.com/docs/schematypes.html)（Mongoose docs）。
...
- 所有[SchemaTypes](https://mongoosejs.com/docs/schematypes.html)都具有內置的[必需](https://mongoosejs.com/docs/api.html#schematype_SchemaType-required)驗證器。這用於指定，是否必須提供該字段才能保存文檔。
- [Numbers](https://mongoosejs.com/docs/api.html#schema-number-js) 數字有最小[min](https://mongoosejs.com/docs/api.html#schema_number_SchemaNumber-min)和最大[max](https://mongoosejs.com/docs/api.html#schema_number_SchemaNumber-max)驗證器。
- [Strings](https://mongoosejs.com/docs/api.html#schema-string-js)字符串有：

  - [enum](https://mongoosejs.com/docs/api.html#schema_string_SchemaString-enum)枚舉：指定該字段的允許值集合。
  - [match](https://mongoosejs.com/docs/api.html#schema_string_SchemaString-match) :指定字符串必須匹配的正則表達式。
  - 字符串的最大長度[maxlength](https://mongoosejs.com/docs/api.html#schema_string_SchemaString-maxlength)和最小長度[minlength](https://mongoosejs.com/docs/api.html#schema_string_SchemaString-minlength)
...
有關字段驗證的完整訊息，請參閱[驗證](https://mongoosejs.com/docs/validation.html)（Mongoose docs）。
...
欲了解更多訊息，請參閱[虛擬](https://mongoosejs.com/docs/guide.html#virtuals)（Mongoose 文檔）。

#### 方法和查詢幫助

綱要 schema 也可以有[實例方法](https://mongoosejs.com/docs/guide.html#methods)，[靜態方法](https://mongoosejs.com/docs/guide.html#statics)和[查詢助手](https://mongoosejs.com/docs/guide.html#query-helpers)。實例和靜態方法很相似，但有明顯的區別，即實例方法與特定記錄相關聯，並且可以訪問當前對象。查詢助手允許你擴展 mongoose 的[鍊式查詢構建器 API](https://mongoosejs.com/docs/queries.html)（例如，除了`find()`, `findOne()`和`findById()`方法外，還允許你添加一個「byName」查詢。

### 使用模型

一旦創建了綱要，就可以使用它來創建模型。該模型代表數據庫中可以搜索的文檔集合，而模型的實例代表你可以保存和檢索的單個文檔。

我們在下面簡要介紹一下。有關更多訊息，請參閱：[模型](https://mongoosejs.com/docs/models.html)（Mongoose docs）。
...
如果你未指定回調，則 API 將返回[Query](https://mongoosejs.com/docs/api.html#query-js)類型的變量。你可以使用此查詢對象來構建查詢，然後稍後使用`exec()`方法執行（使用回調）。
...
[find()](https://mongoosejs.com/docs/api.html#query_Query-find) 方法獲取所有匹配的記錄，但通常你只想獲得一個匹配。以下方法可以查詢單個記錄：

- [`findById()`](https://mongoosejs.com/docs/api.html#model_Model.findById):用指定的`id`查找文檔（每個文檔都有一個唯一的`id`）。
- [`findOne()`](https://mongoosejs.com/docs/api.html#query_Query-findOne): 查找與指定條件匹配的單個文檔。
...
> **備註：** 還有一個[`count()`](https://mongoosejs.com/docs/api.html#model_Model.count)方法，你可以使用它來獲取與條件匹配的項目數。如果你想要在不實際提取記錄的情況下執行計數，這非常有用。

查詢可以做更多的事情。有關更多訊息，請參閱：[查詢](https://mongoosejs.com/docs/queries.html)（Mongoose 文檔）。

#### 運用相關文檔— population 方法

你可以使用`ObjectId`綱要字段，從一個文檔/模型實例，創建一對一引用，或者使用`ObjectIds`數組，從一個文檔創建一對多的引用。該字段存儲相關模型的 ID。如果需要關聯文檔的實際內容，可以在查詢中使用[`populate()`](https://mongoosejs.com/docs/api.html#query_Query-populate)方法，將 id 替換為實際數據。
...
這幾乎是你在本教程中，使用相關項目時，需要了解的所有內容。有關更多詳細訊息，請參閱[Population](https://mongoosejs.com/docs/populate.html)（Mongoose docs）。
...
本教程，我們將使用[mLab](https://mlab.com/welcome/)免費的雲託管的「 [沙盒](https://mlab.com/plans/pricing/) 」數據庫。這個數據庫層不適合生產環境的網站，因為它沒有冗餘設計，但它對於開發和原型設計來說非常有用。我們在這裡使用它，是因為它免費且易於設置，並且因為作為數據庫服務供應商來說，mLab 是流行的數據庫選擇之一，你可能會合理選擇你的生產環境數據庫（撰寫本文時，其他流行的選擇包括[Compose](https://www.compose.com/)、[ScaleGrid](https://scalegrid.io/pricing.html)和[MongoDB Atlas](https://www.mongodb.com/products/platform/atlas-database)）。

> **備註：** 如果你願意，可以下載並安裝[與系統相對應的二進製文件](https://www.mongodb.com/try)，在本地設置 MongoDb 數據庫。除了你在連接時指定的數據庫 URL 之外，本文中的其餘指令將很類似。
