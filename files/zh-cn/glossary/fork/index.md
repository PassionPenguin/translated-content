---
title: 分叉
slug: Glossary/Fork
l10n:
  sourceCommit: 95d5dfa3fa7eac1bdb1a7bfbd67a84c470caa99e
---

{{GlossarySidebar}}

分叉（Fork）是在某一时刻对现有的软件项目进行复制，并在项目加上某人的修改。

基本上，如果原软件的许可证允许，你就可以复制软件的代码，开发自己的版本，附加上自己的补充，这就是一个分叉。

分叉在自由和开源软件的开发中很常见。因为使用 Git 或 Github 平台的贡献模式，分叉是现在比较流行的术语。

## 参见

- 维基百科上的[分叉](<https://zh.wikipedia.org/wiki/分叉_(软件开发)>)
- 在 Git 上下文中：[如何给 GitHub 仓库建立分叉](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/working-with-forks/fork-a-repo)
- 各种“知名的”分叉

  - [Linux 发行版](https://upload.wikimedia.org/wikipedia/commons/1/1b/Linux_Distribution_Timeline.svg)
  - [Node.js 和 io.js（最后又被合并回一起）](https://nodejs.org/en/blog/announcements/foundation-v4-announce/)
  - [LibreOffice，OpenOffice 的一个分叉](https://www.libreoffice.org/about-us/who-are-we/)
