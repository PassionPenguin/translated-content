...
`new.target`属性适用于所有函数访问的元属性。在 [arrow functions](https://www.javascripttutorial.net/es6/javascript-arrow-function/) 中，`new.target` 指向最近的外层函数的`new.target`（An arrow function expression does not have its own this, arguments, super , or new\.target) 。
