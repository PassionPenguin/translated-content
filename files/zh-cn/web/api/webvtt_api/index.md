...
在 Firefox 58 之前，`REGION` 关键字由 {{domxref("VTTRegion")}} 对象创建，但是它们没有被使用。Firefox 58 现在完全支持 `VTTRegion` 以及它的使用；然而，这个功能在首选项 `media.webvtt.regions.enabled` 默认是禁用的；将其设置为 `true` 以在 Firefox 58 开启区域支持。在 Firefox 59 中默认是开启的（参见 bug [Firefox bug 1338030](https://bugzilla.mozilla.org/show_bug.cgi?id%3D1338030) 和 [Firefox bug 1415805](https://bugzilla.mozilla.org/show_bug.cgi?id%3D1415805)）。
