...
拖放的主要步骤是为 [`drop`](/zh-CN/docs/Web/API/HTMLElement/drop_event) 事件定义一个*释放区*(释放文件的目标元素) 和为[`dragover`](/zh-CN/docs/Web/API/HTMLElement/dragover_event)事件定义一个事件处理程序。下面描述了这些步骤，包括示例程序片段。完整的源码在[MDN's drag-and-drop repository](https://github.com/mdn/dom-examples/tree/main/drag-and-drop) (欢迎提交 pull requests 和/或 issues).
