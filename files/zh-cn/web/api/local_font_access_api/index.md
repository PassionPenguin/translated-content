...
`@font-face` 还有其他几个有用的特性。特别是，你还可以在 `local()` 函数中指定字体的完整名称或 Postscript 名称，以告诉浏览器在用户计算机上安装了该字体时使用本地副本。这并非没有问题——`local()` 作为[指纹向量](https://developer.chrome.com/docs/capabilities/web-apis/local-fonts#local-fonts-as-fingerprint-vector)已经变得臭名昭著。
