...
| `TransactionInactiveError` | 如果对不存在的源数据库发出请求（例如，已被删除的）。此外，在 Firefox 版本小于 41 中，会抛出误导性的 `InvalidStateError` 错误，这一问题现已修复（请参阅 [Firefox bug 1176165](https://bugzilla.mozilla.org/show_bug.cgi?id%3D1176165)）。 |
