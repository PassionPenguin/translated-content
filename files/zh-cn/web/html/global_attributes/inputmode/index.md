...
> **警告：** [WHATWG(超文本应用技术工作组) 规范中列出了 `inputmode`](https://html.spec.whatwg.org/multipage/interaction.html#attr-inputmode), 并且现代浏览器正在努力支持它。但 [W3C HTML 5.2 规范](https://html.spec.whatwg.org/multipage/index.html#contents) 不再将其列出（也就是说将其标记为过时的）。在达成共识之前，你应该认为 WHATWG 规范定义的就是正确的。
