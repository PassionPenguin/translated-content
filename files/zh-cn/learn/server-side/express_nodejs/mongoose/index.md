...
本文简要介绍了数据库以及 Node/Express 应用的数据库集成。然后演示了 [Mongoose](https://mongoosejs.com/) 为 [LocalLibrary](/zh-CN/docs/Learn/Server-side/Express_Nodejs/Tutorial_local_library_website) 提供数据库访问的方式。还讲解了对象模式（Schema）和模型（Model）的声明方式、主要域的类型、基础验证机制。同时还简短演示了访问模型数据的一些方法。
...
NPM 站点上有许多 ODM / ORM 解决方案（另请参阅 NPM 站点上的 [odm](https://www.npmjs.com/search?q%3Dkeywords:odm) 和 [orm](https://www.npmjs.com/search?q%3Dkeywords:orm) 标签列表）。

以下是迄今（2018 年 12 月）几种流行的解决方案：

- [Mongoose](https://www.npmjs.com/package/mongoose)：一款为异步工作环境设计的 [MongoDB](https://www.mongodb.com/) 对象建模工具。
- [Waterline](https://www.npmjs.com/package/waterline)：从基于 Express 的 [Sails](https://sailsjs.com/) 框架中提取的 ORM。它提供了一套统一的 API 来访问众多不同的数据库，其中包括 Redis，mySQL，LDAP，MongoDB 和 Postgres。
...
我们将在本地图书馆示例（以及本主题的其余部分）中使用 [Mongoose ODM](https://www.npmjs.com/package/mongoose) 来访问图书馆数据。Mongoose 作为 [MongoDB](https://www.mongodb.com/company/what-is-mongodb)（面向文档数据模型的开源 [NoSQL](https://en.wikipedia.org/wiki/NoSQL) 数据库）的前端。MongoDB 数据库里，“集合”中的“文档” [类似于](https://www.mongodb.com/docs/manual/core/databases-and-collections/#collections) 关系数据库里“表”中的“行”。

这种 ODM 和数据库的结合方式在 Node 社区中非常流行，一定程度上是因为文档存储和查询系统与 JSON 十分相似，因此 JavaScript 开发人员会非常熟悉。

> **备注：** 使用 Mongoose 无需事先了解 MongoDB，但是部分 [Mongoose 文档](https://mongoosejs.com/docs/guide.html) 对于熟悉 MongoDB 的朋友会更易于使用和理解。
...
> **备注：** 本入门受到 npm 上的 [Mongoose 快速入门](https://www.npmjs.com/package/mongoose) 和 [Mongoose 官方文档](https://mongoosejs.com/docs/guide.html) 的“深度影响”。
...
安装 Mongoose 会添加所有依赖项，包括 MongoDB 数据库驱动程序，但不会安装 MongoDB 本身。要安装 MongoDB 服务器，可以 [点击下载](https://www.mongodb.com/try) 各操作系统的安装程序在本地安装。也可以使用云端 MongoDB 实例。
...
大多数 [模式类型](https://mongoosejs.com/docs/schematypes.html)（ [SchemaType](https://mongoosejs.com/docs/schematypes.html)，字段名之后的描述符）都是自解释的。除了：

- `ObjectId`：表示数据库中某一模型的特定实例。例如，一本书可能会使用它来表示其作者对象。它实际只包含指定对象的唯一 ID（`_id`） 。可以使用 `populate()` 方法在需要时提取相关信息。
- [Mixed](https://mongoosejs.com/docs/schematypes.html#mixed)：任意模式类型。
...
关于选项的更多信息请参阅 [模式类型](https://mongoosejs.com/docs/schematypes.html)（Mongoose 英文文档）。
...
- 所有 [模式类型](https://mongoosejs.com/docs/schematypes.html) 都具有内置的 [required](https://mongoosejs.com/docs/api.html#schematype_SchemaType-required) 验证器。用于指定当前字段是否为保存文档所必需的。
- [Number](https://mongoosejs.com/docs/api.html#mongoose_Mongoose-Number) 有数值范围验证器 [min](https://mongoosejs.com/docs/api.html#schema_number_SchemaNumber-min) 和 [max](https://mongoosejs.com/docs/api.html#schema_number_SchemaNumber-max)。
- [String](https://mongoosejs.com/docs/api.html#schema-string-js) 有：

  - [enum](https://mongoosejs.com/docs/api.html#schema_string_SchemaString-enum)：指定当前字段允许值的集合。
  - [match](https://mongoosejs.com/docs/api.html#schema_string_SchemaString-match)：指定字符串必须匹配的正则表达式。
  - 字符串的最大长度 [maxlength](https://mongoosejs.com/docs/api.html#schema_string_SchemaString-maxlength) 和最小长度 [minlength](https://mongoosejs.com/docs/api.html#schema_string_SchemaString-minlength)
...
字段验证的完整信息请参阅 [验证](https://mongoosejs.com/docs/validation.html)（Mongoose 英文文档）。
...
更多信息请参阅 [虚拟属性](https://mongoosejs.com/docs/guide.html#virtuals)（Mongoose 英文文档）。

#### 方法和查询助手

模式支持 [实例方法](https://mongoosejs.com/docs/guide.html#methods)、[静态方法](https://mongoosejs.com/docs/guide.html#statics) 和 [查询助手](https://mongoosejs.com/docs/guide.html#query-helpers)。实例方法和静态方法外表很相似，但有本质区别，实例方法针对特定记录，且可以访问当前对象。查询助手可用于扩展 Mongoose 的 [链式查询 API](https://mongoosejs.com/docs/queries.html)（例如，在 `find()`、`findOne()` 和 `findById()` 方法外还可以添加一个“`byName`”查询）。

### 使用模型

就可以使用创建好的模式来创建模型。模型即数据库中可以搜索的一类文档，模型的实例即可以存取的单个文档。

以下是简介。更多信息请参阅：[模型](https://mongoosejs.com/docs/models.html)（Mongoose 英文文档）。
...
若未指定回调，则 API 将返回 [Query](https://mongoosejs.com/docs/api.html#query-js) 类型的变量。可以使用该查询对象来构建查询，随后使用 `exec()` 方法执行（使用回调）。
...
[`find()`](https://mongoosejs.com/docs/api.html#query_Query-find) 方法会取得所有匹配记录，但通常你只想取得一个。以下方法可以查询单个记录：

- [`findById()`](https://mongoosejs.com/docs/api.html#model_Model.findById)：用指定 `id` 查找文档（每个文档都有一个唯一 `id`）。
- [`findOne()`](https://mongoosejs.com/docs/api.html#query_Query-findOne)：查找与指定条件匹配的第一个文档。
...
> **备注：** 还有一个 [`count()`](https://mongoosejs.com/docs/api.html#model_Model.count) 方法，可获取匹配条件的项目的个数。在只期望获得记录的个数而不想获取实际的记录时可以使用。

查询还能做更多。请参阅 [查询](https://mongoosejs.com/docs/queries.html)（Mongoose 英文文档）。

#### 文档间协同——population

可以使用 `ObjectId` 模式字段来创建两个文档/模型实例间一对一的引用，（一组 `ObjectIds` 可创建一对多的引用）。该字段存储相关模型的 id。如果需要相关文档的实际内容，可以在查询中使用 [`populate()`](https://mongoosejs.com/docs/api.html#query_Query-populate) 方法，将 id 替换为实际数据。
...
以上是本教程中“项目间协同”需要了解的所有内容。更多详细信息请参阅 [Population](https://mongoosejs.com/docs/populate.html)（Mongoose 英文文档）。
...
本教程将使用 [mLab](https://mlab.com/welcome/) 免费版“[沙盒](https://mlab.com/plans/pricing/)”云数据库。这一版不适用于生产环境，因为它没有冗余设计，但非常适合进行开发和原型设计。选用它是因为它免费且易于设置，并且 mLab 是一家流行的数据库服务供应商，也是生产环境数据库的理想选择（撰写本文时（2019 年 1 月），国内流行的云数据库解决方案有 [阿里云](https://cn.aliyun.com/product/mongodb?from_alibabacloud%3D%26spm%3D5176.10695662.778269.1.2e5b8cb3Hw9HUr)、[腾讯云](https://cloud.tencent.com/product/mongodb)、[百度云](https://cloud.baidu.com/product/mongodb.html) 等）。

> **备注：** 也可以下载并安装 [对应系统的安装包](https://www.mongodb.com/try)，设置本地版 MongoDB 数据库。多数指令和使用云数据库是一样的，除了连接时数据库的 URL。
...
- [Mongoose 站点](https://mongoosejs.com/) (Mongoose 文档)
- [Mongoose 指南](https://mongoosejs.com/docs/guide.html) (Mongoose 文档)
- [验证](https://mongoosejs.com/docs/validation.html) (Mongoose 文档)
- [模式类型](https://mongoosejs.com/docs/schematypes.html) (Mongoose 文档)
- [模型](https://mongoosejs.com/docs/models.html) (Mongoose 文档)
- [查询](https://mongoosejs.com/docs/queries.html) (Mongoose 文档)
- [填充](https://mongoosejs.com/docs/populate.html) (Mongoose 文档)
