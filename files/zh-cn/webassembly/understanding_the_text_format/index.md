...
如果我们现在把该模块转换为二进制（参考[把 WebAssembly 文本格式转换为 wasm](/zh-CN/docs/WebAssembly/Text_format_to_wasm)），我们将会看到在[二进制格式](https://webassembly.github.io/spec/core/binary/index.html#high-level-structure)中描述的 8 字节的模块头：
...
- 在当前版本中，最多拥有一个返回类型，但是[以后会放开这个限制](https://webassembly.org/features/#multiple-return)到任意数量。
...
有很多东西都可以放在函数体里面，但是，现在我们从简单的开始，然后随着逐步前进，你会看到更多的例子。访问 [webassembly.org 语义手册](https://webassembly.github.io/spec/core/exec/index.html)获取可用操作码的完整列表。
...
按照 WebAssembly 的定义，内存就是一个随着时间增长的字节数组。WebAssembly 包含诸如 i32.load 和 i32.store 指令来实现对[线性内存](https://webassembly.github.io/spec/core/exec/index.html#linear-memory)的读写。
...
因为表格是可变的，所以，它们能够用来实现复杂的加载时和运行时[动态链接](https://github.com/WebAssembly/tool-conventions/blob/main/DynamicLinking.md)。当程序被动态地链接，多个实例共享相同的内存和表格。这与原生应用程序的多个.dll 共享一个进程地址空间是等价的。
